// apperror.go provides the single error type that travels from the
...
// NewError builds an error with an explicit code and status.
func NewError(code Code, status int, message string) *Error {
	return &Error{Code: code, Status: status, Message: message}
}

func BadRequest(message string) *Error {
	return NewError(CodeBadRequest, http.StatusBadRequest, message)
}
...
func NotFound(resource string) *Error {
	...
	return NewError(CodeNotFound, http.StatusNotFound, resource+" not found")
}
...
func TooManyRequests(message string) *Error {
	...
	return NewError(CodeTooManyRequests, http.StatusTooManyRequests, message)
}
...
func UnsupportedMediaType(message string) *Error {
	...
	return NewError(CodeUnsupportedType, http.StatusUnsupportedMediaType, message)
}
...
func Unavailable(message string, err error) *Error {
	...
	return NewError(CodeUnavailable, http.StatusServiceUnavailable, message).Wrap(err)
}
