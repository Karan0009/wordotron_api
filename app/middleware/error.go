package middleware

import (
	"context"
	"errors"
	"log/slog"

	"github.com/gofiber/fiber/v3"

	"github.com/Karan0009/wordotron_api/app/lib"
)

// ErrorHandler is the single place where an error becomes an HTTP response.
// Handlers and services return errors; nobody writes an error body by hand.
func ErrorHandler(log *slog.Logger) fiber.ErrorHandler {
	return func(c fiber.Ctx, err error) error {
		appErr := translate(err)

		attrs := []slog.Attr{
			slog.String("method", c.Method()),
			slog.String("path", c.Path()),
			slog.Int("status", appErr.Status),
			slog.String("code", string(appErr.Code)),
			slog.String("error", err.Error()),
		}

		if appErr.Status >= fiber.StatusInternalServerError {
			log.LogAttrs(c.Context(), slog.LevelError, "request failed", attrs...)
		} else {
			log.LogAttrs(c.Context(), slog.LevelDebug, "request rejected", attrs...)
		}

		return c.Status(appErr.Status).JSON(lib.ErrorEnvelope{
			Success:   false,
			Message:   appErr.Message,
			Code:      appErr.Code,
			Errors:    appErr.Fields,
			RequestID: RequestIDFrom(c),
		})
	}
}

// translate normalises framework and runtime errors into *lib.Error so
// the response shape never depends on where the failure originated.
func translate(err error) *lib.Error {
	if appErr, ok := lib.As(err); ok {
		return appErr
	}

	// Fiber reports routing, body-limit and parser failures as *fiber.Error.
	var fiberErr *fiber.Error
	if errors.As(err, &fiberErr) {
		switch fiberErr.Code {
		case fiber.StatusNotFound:
			return lib.NewError(lib.CodeNotFound, fiber.StatusNotFound, "The requested endpoint does not exist")
		case fiber.StatusMethodNotAllowed:
			return lib.NewError(lib.CodeBadRequest, fiber.StatusMethodNotAllowed, "Method not allowed for this endpoint")
		case fiber.StatusRequestEntityTooLarge:
			return lib.PayloadTooLarge("")
		case fiber.StatusUnprocessableEntity, fiber.StatusBadRequest:
			return lib.BadRequest("The request body could not be parsed")
		default:
			return lib.NewError(lib.CodeInternal, fiberErr.Code, fiberErr.Message).Wrap(err)
		}
	}

	// A cancelled client connection is not a server fault.
	if errors.Is(err, context.Canceled) {
		return lib.NewError(lib.CodeBadRequest, 499, "Client closed the request").Wrap(err)
	}
	if errors.Is(err, context.DeadlineExceeded) {
		return lib.NewError(lib.CodeUnavailable, fiber.StatusGatewayTimeout, "The request timed out").Wrap(err)
	}

	return lib.Internal(err)
}
